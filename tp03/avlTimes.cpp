...
No *construtorNo(Time *elemento);
No *setNivel(No *esse);
int getNivel(No *no);
AVL *construtorAVL();
int getAltura(AVL *arvore);
bool pesquisar(AVL *arvore, char *elemento);
bool pesquisarPrivado(No *no, char *x);
void mostrarCentral(AVL *arvore);
void mostrarCentral(No *no);
void mostrarPre(AVL *arvore);
void mostrarPre(No *no);
void inserir(AVL *arvore, Time *x);
No *inserir(No *no, Time *x);
No *balancear(No *no);
No *rotacionarDir(No *no);
No *rotacionarEsq(No *no);
void remover(AVL *arvore, char *x);
No *remover(No *no, char *x);
...
int main()
{
	...
	for (int i = 0; i < quantidadeEntradas; i++)
	{
		...
		inserir(arvoreAvl, conjuntoTimes[i]);
	}

	//mostrarCentral(arvoreAvl);
	...
	for (int i = 0; i < quantidadeEntradas; i++)
	{
		if (pesquisar(arvoreAvl, listaPesquisa[i]))
			printf(" SIM\n");
		else
			printf(" NÃO\n");
	}
...
bool pesquisar(AVL *arvore, char *elemento)
{
	...
	bool resp = pesquisarPrivado(arvore->raiz, elemento); 
	return resp;
}
...
bool pesquisarPrivado(No *no, char *x)
{
	...
	{
		int indiceComparacao = strcmp(x, no->elemento->nome);
		if (indiceComparacao == 0)
		{
			resp = true;
		}
		else if (indiceComparacao < 0)
		{
			printf(" esq");
			//printf(" esq %s\n", no->elemento->nome);
...
void mostrarCentral(No *no)
{
	if (no != NULL)
	{
		...
		printf("%s\n", no->elemento->nome); // Conteudo do no->
...
void mostrarPre(No *no)
{
	if (no != NULL)
	{
		printf("%s (f %d)\n", no->elemento->nome, (getNivel(no->dir) - getNivel(no->esq))); // Conteudo do no->
...
No *inserir(No *no, Time *x)
{
	...
	{
		//printf("Erro ao inserir elemento (%s)!\n", x->nome);
	}
