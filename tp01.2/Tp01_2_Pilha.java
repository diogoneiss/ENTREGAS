...
class PilhaTime{
    ...
    private int n;
    private Time[] pilhaDosTimes;

    public void inserir(Time entrada) throws Exception{
        if(pilhaDosTimes.length == n)
            throw new Exception("Tamanho do arranjo excedido!");
        //adiciona em n e, em seguida, incrementa n
        pilhaDosTimes[n++] = entrada;
    }
   
    public Time remover() throws Exception{
        if(n == 0)
            throw new Exception("Tamanho do arranjo excedido!");
        //pega o elemento na posicao n-1, já que n não é exatamente uma posicao;
        Time retorno = pilhaDosTimes[--n];
    ...
    PilhaTime(int num){
        this.pilhaDosTimes = new Time[num+1];
        this.n = 0;
    }
...
public class Tp01_2_Pilha {
    public static void main(String[] args) throws Exception{
        ...
        PilhaTime pilhaDosTimes = new PilhaTime(quantidadeDeFrases + quantidadeModificacoes);


        // adicionar os times mencionados previamente no fim da lista
        for (int i = 0; i < quantidadeDeFrases; i++) {
            pilhaDosTimes.inserir(conjuntoTimes[i]);
        }
...
//        listaDosTimes.mostrar();
        ...
        for(int j = 0; j< quantidadeModificacoes; j++){
            ...
            if(operacoes[j].charAt(0) == 'I'){
                ...
                Time novaEntrada = new Time(operacoes[j].substring(2));
                pilhaDosTimes.inserir(novaEntrada);
            ...
            //metodos de remocao
                ...
                Time removido = pilhaDosTimes.remover();
