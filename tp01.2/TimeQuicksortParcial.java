...
class TimeQuicksortParcial {
    ...
    public static void main(String[] args) throws Exception {
        ...
        TimeQuicksortParcial[] conjuntoTimes = new TimeQuicksortParcial[quantidadeDeFrases];
        
        //criar os objetos de acordo.
        for (int i = 0; i < quantidadeDeFrases; i++) {
            //criar o objeto e chamar o construtor
            conjuntoTimes[i] = new TimeQuicksortParcial(entradas[i]);
        }
        
        int totalComparacoes = ordenarQuick(conjuntoTimes);
    ...
    public static void swap(TimeQuicksortParcial array[], int menor, int i){
        TimeQuicksortParcial aux = array[i];
        array[i] = array[menor];
        array[menor] = aux;
    ...
    public static int ordenarQuick(TimeQuicksortParcial array[]){
        int comparacoes = 0;

        comparacoes += quicksort(array, 0, array.length-1);

        return comparacoes;
    }

    public static int quicksort(TimeQuicksortParcial array[], int esq, int dir) {
        int i = esq, j = dir, contador = 0;
        
        TimeQuicksortParcial pivo = array[(dir+esq)/2];
        ...
        if (esq < j)
            contador += quicksort(array, esq, j);
        if (i < dir)
            contador += quicksort(array, i, dir);
