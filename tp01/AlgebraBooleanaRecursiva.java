public class AlgebraBooleanaRecursiva {
    ...
    /*
     * Essa funcao serve para simplificar NOTs unicos no codigo Exemplo: "not(0)"
     */
    public static String simplificarNOT(String fraseOriginal) {
        char bitDaOperacao;

        // ultima posicao da string
        int tam = fraseOriginal.length() - 1;
        ...
        for (int i = tam; i >= 5; i--) {
            ...
            // length() > 2 é pra evitar o acesso do elemento i-1 caso haja apenas um
            // elemento
            // i < length() é pra quando houber reescrita de string nao tentar acessar um
            // indice proibido
            boolean podeSimplificar = i < fraseOriginal.length() && fraseOriginal.length() > 2
                    && fraseOriginal.charAt(i) == ')' && fraseOriginal.charAt(i - 1) != ')';

            // operacao not e seus metodos
            if (podeSimplificar && fraseOriginal.charAt(i - 3) == 't') {
    ...
    /*
     * Essa funcao vai servir para simplificar a operacao iterativamente, ate haver
     * simplificado o bastante Para isso, vai andar na string de tras pra frente,
     * retirar uma operacao e substituir por 0 ou 1
     */
    public static String simplificarOperacao(String fraseOriginal) {
        ...
        while (fraseOriginal.length() > 2 && iteracoes < 10) {
            ...
            for (int i = fraseOriginal.length() - 1; i >= 4; i--) {
                ...
                /*
                 * ATENCAO Para passar a funcao reformatarString, é necessário passar a primeira
                 * letra da operacao ('a', por exemplo) e o parenteses de fechamento, já que a
                 * funcao usa as duas posicoes como demarcadores, nao os incluindo
                 */

                // ja achei o parenteses de abertura e o de fechamento. Posso seguir com a
                // respectiva operacao

                // operacao and
                if (podeSimplificar && parentesesAchado && fraseOriginal.charAt(posicaoAberturaParenteses - 1) == 'd'
                        && fraseOriginal.charAt(posicaoAberturaParenteses - 2) == 'n'
                        && fraseOriginal.charAt(posicaoAberturaParenteses - 3) == 'a') {

                    bitDaOperacao = operacaoAND(
                            cortarString(fraseOriginal, posicaoAberturaParenteses, posicaoFechamentoParenteses));
                    fraseOriginal = reformatarString(fraseOriginal, posicaoAberturaParenteses - 3,
                            posicaoFechamentoParenteses, bitDaOperacao);
                }

                // operacao or
                else if (podeSimplificar && parentesesAchado
                        && fraseOriginal.charAt(posicaoAberturaParenteses - 1) == 'r'
                        && fraseOriginal.charAt(posicaoAberturaParenteses - 2) == 'o') {

                    // cortaString() deixa apenas o conteudo entre os parenteses
                    bitDaOperacao = operacaoOR(
                            cortarString(fraseOriginal, posicaoAberturaParenteses, posicaoFechamentoParenteses));
                    fraseOriginal = reformatarString(fraseOriginal, posicaoAberturaParenteses - 2,
                            posicaoFechamentoParenteses, bitDaOperacao);
                }
                // operacao not
                else if (podeSimplificar && parentesesAchado
                        && fraseOriginal.charAt(posicaoAberturaParenteses - 1) == 't'
                        && fraseOriginal.charAt(posicaoAberturaParenteses - 2) == 'o'
                        && fraseOriginal.charAt(posicaoAberturaParenteses - 3) == 'n') {
    ...
    /*
     * serve para transformar a string original, com A B e etcs em uma so com a
     * expressao booleana, alem de cortar fora o começo da string, deixando APENAS
     * as instruções lógicas para evaluacao
     * 
    ...
    public static String substituirLetrasExpressao(String frase) {
        ...
        // substituir na string as letras maiusculas pelos seus respectivos valores
        // booleanos

        /*
         * 1 0 not(A) 0123456789ABCDEF 2 0 0 or( A , B)
         * 
         * 2a posicao = valor do A 4a posicao = valor do B 6a posicao = valor do C
         */
        ...
        // ja substitui os valores booleanos. agora falta cortar a string, removendo os
        // valores booleanos de inicio
    ...
    /*
     * Essa funcao vai servir para dividir uma string em duas, remover o espaco
     * entre duas posicoes e inserir um unico char, da reposta. AnteriorFim e
     * posteriorInicio estao sendo excluidas nas strings
     */
    public static String reformatarString(String fraseOriginal, int anteriorFim, int posteriorInicio, char substituir) {
        ...
        // adicionar as strings juntas agora, fazendo anterior + meio + fim, sendo o
        // meio um unico char
    ...
    */
    public static char operacaoAND(String frase) {
        return operacaoAND(frase, 0);
    }
    private static char operacaoAND(String frase, int pos) {
        char resultado = '1';
        // iterar a string pra procurar zeros
        if(pos < frase.length()) {
            // se houver qualquer 0 o AND retornara false
            if (frase.charAt(pos) == '0') {
                resultado = '0';
            }
            else
                resultado = operacaoAND(frase, pos+1);
        }
        return resultado;
    }
    ...
    */
    //versão recursiva
    public static char operacaoOR(String frase){
        return operacaoOR(frase, 0);
    }
    private static char operacaoOR(String frase, int pos){
        char resultado = '0';
        if(pos < frase.length()){
            if(frase.charAt(pos) == '1'){
                ...
                resultado = operacaoOR(frase, pos+1);
        }
    ...
    // le se eh o final da entrada, para terminar os metodos de entrada e começar a
